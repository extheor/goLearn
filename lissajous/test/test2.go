...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		// strRand := fmt.Sprintf("%01v", rand.New(rand.NewSource(time.Now().UnixNano())).Int31n(10)+1)

		// randNum, _ := strconv.ParseFloat(strRand, 10)

		// fmt.Println("输出: ", fmt.Sprintf("%X", math.Floor(randNum*0xffffff)))
